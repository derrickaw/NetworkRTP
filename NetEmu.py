...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(args) < 1:
            parser.error("Argument port is required")
        if len(args) > 1:
            parser.error("Only 1 argument is required")
        port = int(args[0])
        if not(100 < port < 20000):
            parser.error("Port number should be between 100 and 20000")
    except ValueError:
